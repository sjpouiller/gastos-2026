exports.handler = async function(event) {
  ...
  <div style="max-width:520px;margin:32px auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,.08)">
    <div style="background:#1a1a1a;padding:24px 28px">
      <div style="color:#fff;font-size:22px;font-weight:700">💰 Plata Clara</div>
    ...
    <div style="padding:28px;font-size:14px;color:#333;line-height:1.8">
      <p style="margin:0 0 16px"><strong>${nombreInvitante}</strong> te invitó a unirte al hogar <strong>${nombreHogar}</strong> en Plata Clara, la app para gestionar gastos en pareja o familia.</p>
    ...
    <div style="padding:16px 28px;border-top:1px solid #e5e5e0;font-size:11px;color:#aaa;text-align:center">
      Plata Clara · <a href="https://plataclara.netlify.app" style="color:#aaa">plataclara.netlify.app</a>
    </div>
  </div>
</body></html>`;
    ...
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: [emailInvitado],
        subject: `💰 ${nombreInvitante} te invitó a Plata Clara`,
        html
      })
    });
